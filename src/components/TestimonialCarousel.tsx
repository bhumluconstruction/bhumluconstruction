      ...
      <div className="relative flex w-full overflow-hidden group">
        ...
        <div className="flex space-x-6 animate-marquee group-hover:[animation-play-state:paused] min-w-full flex-shrink-0">
          {doubledTestimonials.map((item, index) => (
            <div
              key={`${item.id}-${index}`}
              className="w-[350px] md:w-[420px] bg-slate-800/80 backdrop-blur-md border border-slate-700/60 rounded-xl p-8 flex flex-col justify-between hover:border-amber-500/50 transition-all duration-300 shadow-xl flex-shrink-0"
            >
              <div>
